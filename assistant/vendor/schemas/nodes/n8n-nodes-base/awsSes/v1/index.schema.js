/**
 * AWS SES Node - Version 1 - Zod Schema Factory
 * Exports a factory that unions all discriminator schemas.
 *
 * Schema helpers (z, expressionSchema, etc.) are passed as parameters
 * by the schema-validator, not imported from external files.
 *
 * @generated - CommonJS JavaScript for runtime loading
 */

const getCustomVerificationEmailSchema = require('./resource_custom_verification_email/index.schema');
const getEmailSchema = require('./resource_email/index.schema');
const getTemplateSchema = require('./resource_template/index.schema');

module.exports = function getSchema(helpers) {
  const { parameters, z } = helpers;
  // Apply discriminator default if not set
  const effectiveParams = parameters.resource === undefined ? { ...parameters, resource: 'email' } : parameters;
  return z.union([
    getCustomVerificationEmailSchema({ ...helpers, parameters: effectiveParams }),
    getEmailSchema({ ...helpers, parameters: effectiveParams }),
    getTemplateSchema({ ...helpers, parameters: effectiveParams }),
  ]);
};