...
2. **An AI provider API key.** OpenAI, Anthropic (Claude), or Google Gemini — whichever you already have. If you have none, OpenAI is the fastest to sign up for. Typical cost to test is a few cents.
...
1. **AI provider.** Open the AI node, create a **Header Auth** credential, and paste your API key (`Authorization: Bearer YOUR_KEY`). The setup guide lists the suggested model — use it for your first run.
