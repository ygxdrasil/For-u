...
const FILE = process.argv.find((a) => a.startsWith('--file='))?.split('=')[1]
  ?? 'trading/cache/era-results.json';
...
console.log(`\n${'='.repeat(80)}`);
console.log(`ERA ANALYSIS — ${rows.length} backtests, ${eras.length} non-overlapping regimes`);
...
// ---------------------------------------------------------------------------
console.log(`\n\nPOOLED ACROSS ALL MARKETS — one row = one strategy, one era, one timeframe`);
...
// ---------------------------------------------------------------------------
console.log(`\n\nCONSISTENCY — does the edge survive EVERY era?\n`);
const w2 = [30, 4, ...eras.map(() => 11), 14];
...
for (const s of strategies) {
  for (const tf of tfs) {
    ...
    console.log([
      ...
      pad(tf, w2[1]),
      ...cells.map((c, i) => pad(c ? `${c.point >= 0 ? '+' : ''}${c.point.toFixed(2)}${c.edge ? '*' : c.hi < 0 ? '!' : ' '}` : '—', w2[2 + i])),
      edges === cells.filter(Boolean).length && edges > 0 ? `ROBUST ${edges}/${edges}`
        : edges > 0 ? `${edges}/${cells.filter(Boolean).length} only`
        : loses === cells.filter(Boolean).length && loses > 0 ? 'ALWAYS LOSES' : 'no edge',
    ].join(' '));
  }
}
...
// ---------------------------------------------------------------------------
const REF = rows.some((r) => r.product === 'BTC-USD') ? 'BTC-USD' : 'S&P 500';
console.log(`\n\nINDEPENDENCE CHECK — ${REF} alone vs all markets pooled\n`);
for (const s of strategies.slice(0, 3)) {
  for (const tf of ['1d']) {
    const btc = rows.filter((r) => r.strategy === s && r.timeframe === tf && r.product === REF).flatMap((r) => r.rs);
    ...
    console.log(`  ${pad(s.slice(0, 30), 32)} ${REF} only: ${b.point >= 0 ? '+' : ''}${b.point.toFixed(3)} (${btc.length} tr)   pooled: ${a.point >= 0 ? '+' : ''}${a.point.toFixed(3)} (${all.length} tr)`);
  }
}
console.log(`\n  Markets within an asset class are correlated, so N markets is NOT N independent bets.`);
...
const robust = strategies.flatMap((s) =>
  tfs.map((tf) => {
    const cells = eras.map((e) => pooled.find((p) => p.strategy === s && p.tf === tf && p.era === e.name)).filter(Boolean);
    const edges = cells.filter((c) => c!.edge).length;
    return { s, tf, edges, covered: cells.length };
  }),
).filter((x) => x.covered >= 3 && x.edges === x.covered);
...
console.log(`\nStrategies with an edge in EVERY era they covered (min 3): ${robust.length}`);
for (const r of robust) console.log(`   - ${r.s} (${r.tf}) ${r.edges}/${r.covered} eras`);
